...
export const useCadastroFacial = (): UseCadastroFacialReturn => {
  ...
  const [statusMessage, setStatusMessage] = useState(
    "Sistema pronto - Preencha os dados e inicie o reconhecimento",
  );
  ...
  const { validateCadastroForm, validateDescriptor, getDistanceMessage, showValidationErrors } =
    useValidation();
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (verificacao.existe) {
          const nomeExistente =
            (verificacao.dados?.usuario as { nome?: string })?.nome || "Usuário desconhecido";
        ...
        const response = await authenticatedFetch(
          `${baseURL}/usuarios/listar?nome=${encodeURIComponent(nome.trim())}`,
          {
            method: "GET",
            headers: {
        ...
        const nomeJaExiste =
          ...
          usuarios.some((usuario) => usuario.nome.toLowerCase() === nome.trim().toLowerCase());
