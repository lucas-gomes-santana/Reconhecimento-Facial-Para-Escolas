...
export const useAdminPage = () => {
  ...
  const { authenticatedFetch, isAdmin, isDesenvolvedor, isSuperAdmin, admin } = useAuth();
  ...
  const handleCadastrarAdmin = useCallback(
  ...
  const carregarAdmins = useCallback(
  ...
  const atualizarAdminsExibidos = useCallback(() => {
    ...
    if (searchTerm.trim()) {
      adminsFiltrados = todosAdmins.filter(
        (admin) =>
