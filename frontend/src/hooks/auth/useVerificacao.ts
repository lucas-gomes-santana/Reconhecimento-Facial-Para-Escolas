...
export const useVerificacao = () => {
  const { clearError, error: apiError, setError, setLoading, handleApiError } = useApi();
  ...
  const { validateDescriptor, showValidationErrors } = useValidation();

  const { authenticatedFetch } = useAuth();

  const [isInitialized, setIsInitialized] = useState(false);
  const [verificacaoCompleta, setVerificacaoCompleta] = useState(false);
  const [resultadoVerificacao, setResultadoVerificacao] = useState<VerificarRostoResponse | null>(
    null,
  );
    ...
    try {
      ...
      if (currentDescriptor && currentDescriptor.length > 0) {
        ...
        try {
          ...
          console.error("Tempo esgotado, tente novamente.", timeoutError);
          return;
        }
      }
      ...
      if (!validation.isValid) {
        console.error("Erro na captura biométrica. Tente novamente.");
  ...
  const verificarRosto = useCallback(
    async (descriptor: number[], contexto: string): Promise<VerificarRostoResponse> => {
      setLoading(true);
      setError(null);
